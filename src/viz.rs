...
pub fn generate_plots(csv_path: &str) -> Result<()> {
    ...
    for result in rdr.deserialize() {
        ...
        if record.status == "Correct" {
            entry.0 += 1;
            durations.entry(record.library.clone()).or_default().push(record.duration_us);
        }
    }
...
fn draw_success_rates(categories: &[String], libraries: &[String], stats: &HashMap<(String, String), (u32, u32)>) -> Result<()> {
    ...
    for (lib_idx, lib) in libraries.iter().enumerate() {
        ...
        chart.draw_series(
            (0..categories.len()).map(|cat_idx| {
                ...
                Rectangle::new([(x0, 0.0), (x1, rate)], color.filled())
            })
        )?.label(lib)
...
fn draw_performance(libraries: &[String], durations: &HashMap<String, Vec<u64>>) -> Result<()> {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("Performance (Median Duration - Correct Decodes)", ("sans-serif", 30))
        .margin(20)
    ...
    for (idx, (_lib, median, _p95)) in perf_stats.iter().enumerate() {
        ...
        chart.draw_series(std::iter::once(
            Rectangle::new(
                [(SegmentValue::CenterOf(idx), 0), (SegmentValue::CenterOf(idx), *median)],
                color.filled()
            )
        ))?;
    }
